...
func (info *ThriftUsedInfo) ParseThriftIdl() (rawStructs []*IdlExtractStruct, err error) {
	...
	getGenGoFilePath = func(file *parser.Thrift) error {
		...
		importPath := filepath.Join(info.ModelArgs.ModelPkgName,
		...
		info.ImportPaths = append(info.ImportPaths, importPath) // 将 importPath 添加到 ImportPaths 中
		...
		for _, st := range file.Structs {
			...
			if hasInterface {
				...
				if len(rawStruct.StructFields) != 0 {
					...
					if err = rawStruct.recordMysqlIfInfo(info.ModelArgs.DaoDir); err != nil {
						return err
					}
...
func extractIdlStruct(st *parser.StructLike, file *parser.Thrift, rawStruct *IdlExtractStruct) error {
	for _, field := range st.Fields {
		...
		if len(field.Annotations) > 0 && fag != nil && strings.Contains(fag[0], consts.ThriftSql) {
...
func AddMysqlModelImports(data string, impt []string) (string, error) {
